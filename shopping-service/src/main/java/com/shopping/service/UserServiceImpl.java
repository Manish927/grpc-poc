...
public class UserServiceImpl extends UserServiceGrpc.UserServiceImplBase {
    ...
    public void getUserDetails(UserRequest request, StreamObserver<UserResponse> responseObserver) {
        ...
        List<Order> orders = getOrdersFromService(userResponseBuilder);

        userResponseBuilder.setNumOfOrders(orders.size());
        UserResponse userResponse = userResponseBuilder.build();
        responseObserver.onNext(userResponse);
        responseObserver.onCompleted();
    }

    private List<Order> getOrdersFromService(UserResponse.Builder userResponseBuilder) {
        //get orders by invoking the order client
        logger.info("Creating channel and calling the order Client via channel" );
